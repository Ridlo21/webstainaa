...
<main id="main">
    ...
    <section id="portfolio" class="portfolio">
        <div class="container">
            ...
            <div class="row portfolio-container" data-aos="fade-up">
                <?php
                $r = $this->db->from('berita')->order_by('id_berita', 'DESC')->limit(3)->get()->result_array();
    ...
    <section id="portfolio" class="portfolio">
        <div class="container">
            ...
            <div class="row" data-aos="fade-up">
                <div class="col-lg-12 d-flex justify-content-center">
                    <ul id="portfolio-flters-artikel">
                        <li data-filter=".filter-himasta" class="but-artikel filter-active ">HIMASTA</li>
                        <li data-filter=".filter-himasya">HIMASYA</li>
                    </ul>
                </div>
            </div>

            <div class="row portfolio-container-artikel" data-aos="fade-up">
                <?php
                $himasta = $this->db->from('artikel')->where('nama_himpunan', 'HIMASTA')->order_by('idartikel', 'DESC')->limit(3)->get()->result_array();
                ?>
                <?php foreach ($himasta as $hmt) { ?>
                    <div class="col-lg-4 col-md-6 portfolio-item filter-himasta">
                        <article class="entry entry-single mt-3">

                            <div class="entry-img">
                                <img src="<?= base_url() ?>asset/gambar_artikel/foto_artikel/<?= $hmt['gambar'] ?>" alt="" class="img-fluid">
                            </div>

                            <h5 class="entry-title" style="font-size: medium !important;">
                                <a href="<?= base_url() ?>artikel/detail/<?= $hmt['nama_himpunan'] ?>/<?= $hmt['judul_seo'] ?>"><?= $hmt['judul_artikel'] ?> </a>
                            </h5>

                            <div class="entry-meta">
                                <ul>
                                    <li class="d-flex align-items-center"><i class="bi bi-person"></i> <a><?= $hmt['penulis'] ?></a></li>
                                </ul>
                            </div>
                        </article>
                    </div>
                ...
                <?php
                $himasya = $this->db->from('artikel')->where('nama_himpunan', 'HIMASYA')->order_by('idartikel', 'DESC')->limit(3)->get()->result_array();
                ?>
                <?php foreach ($himasya as $hmy) { ?>
                    <div class="col-lg-4 col-md-6 portfolio-item filter-himasya">
                        <article class="entry entry-single mt-3">

                            <div class="entry-img">
                                <img src="<?= base_url() ?>asset/gambar_artikel/foto_artikel/<?= $hmy['gambar'] ?>" alt="" class="img-fluid">
                            </div>

                            <h5 class="entry-title" style="font-size: medium !important;">
                                <a href="<?= base_url() ?>artikel/detail/<?= $hmy['nama_himpunan'] ?>/<?= $hmy['judul_seo'] ?>"><?= $hmy['judul_artikel'] ?> </a>
                            </h5>

                            <div class="entry-meta">
                                <ul>
                                    <li class="d-flex align-items-center"><i class="bi bi-person"></i> <a><?= $hmy['penulis'] ?></a></li><br>
                                </ul>
                            </div>
                        </article>
                    </div>
