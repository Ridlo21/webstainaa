...
class Model_menu extends CI_model
{
    ...
    function menu_idn_update()
    {
        $datadb = array(
            'nama' => $this->db->escape_str($this->input->post('a'))
        );
        $this->db->where('id_idn', $this->input->post('id'));
        $this->db->update('idn', $datadb);
    }

    function idn_edit($id)
    {
        return $this->db->query("SELECT * FROM idn where id_idn='$id'");
    }

    function idn_delete($id)
    {
        return $this->db->query("DELETE FROM idn where id_idn='$id'");
    }
}
